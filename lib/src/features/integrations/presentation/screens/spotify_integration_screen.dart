...
class SpotifyIntegrationScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Integracion Spotify'),
        actions: [
          IconButton(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (!isConnected)
            FilledButton.icon(
              onPressed: statusAsync.isLoading || isLinking
                  ? null
                  : () => notifier.linkWithSpotifyOAuth(),
              icon: const Icon(Icons.login_rounded),
              label: const Text('Iniciar sesion con Spotify'),
            ),
          if (isConnected) ...[
            FilledButton.icon(
              onPressed: () => context.push(AppRoutes.spotifyPlaylists),
              icon: const Icon(Icons.queue_music_rounded),
              label: const Text('Ver playlists'),
            ),
            const SizedBox(height: 12),
            OutlinedButton.icon(
              onPressed: () => notifier.unlinkAccount(),
              icon: const Icon(Icons.link_off_rounded),
              label: const Text('Desvincular Spotify'),
            ),
          ],
          const SizedBox(height: 12),
          OutlinedButton.icon(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            SizedBox(width: 12),
            Expanded(child: Text('Consultando estado de Spotify...')),
          ],
        ),
      ),
    );
